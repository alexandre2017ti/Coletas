...
public sealed record AccountProfile(Guid UserId, string Email, UserRole Role, UserStatus Status, Guid? CourierId,
    IReadOnlyList<VehicleResponse> Vehicles, IReadOnlyList<DocumentProfile> Documents, string? Reason);
...
public sealed record DocumentDecision(CourierDocumentStatus Status, string Reason);
